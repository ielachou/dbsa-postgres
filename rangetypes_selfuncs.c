...
static double estimate_strictly_left(AttStatsSlot pres, RangeBound const_upper, RangeBound const_lower, int depth, Datum lower_value, int count);
	...
	switch (operator)
	{
		...
		case OID_RANGE_LEFT_OP:
			...
			hist_selec =
				...
				hist_selec = estimate_strictly_left(pres, const_upper, const_lower, ((upper_value - lower_value) / pres.nvalues), lower_value, count);
...
static double estimate_strictly_left(AttStatsSlot pres, RangeBound const_upper, RangeBound const_lower, int depth, Datum lower_value, int count){
	int currentLower, currentUpper;
	double selec = 0;
	int index = -1;
	int diff;
	double proportion, proba, tmp, random;
	int pop[2];
	double probas[2];
	srand(time(NULL));

	for(int i = 0; i < pres.nvalues; i++){
		
		currentLower = (depth * i) + DatumGetInt32(lower_value);
        currentUpper = (depth * i) + DatumGetInt32(lower_value) + depth;
		if( const_lower.val <  currentUpper && const_lower.val >= currentLower ){
			index = i;
			break;
		}
	}
	...
	for(int j  = 0; j < pres.nvalues-1; j++){
		...
		if(j == index){
			...
			currentLower = (depth * j) + DatumGetInt32(lower_value);
        	currentUpper = (depth * j) + DatumGetInt32(lower_value) + depth;


			if(pres.values[j] < pres.values[j+1]){
				diff = currentUpper + depth - const_lower.val;
				proportion = (double) diff/ (double) depth;
				proba = 1 - proportion;
				tmp = proba * (pres.values[j+1] - pres.values[j]);
				selec += round(tmp);
		...
		printf("values[j], j+1 : %d, %d\n", pres.values[j], pres.values[j+1] );

		if(pres.values[j] < pres.values[j+1] && j+1 < index){
			//Why ? Because it increases we have more chance
			//that the past ranges are extended to the current one
			random = (double) rand()/(double) RAND_MAX;
			if(random < 0.85){
				selec+=0;
			}else{
				selec+=(pres.values[j+1] - pres.values[j]) /2;
			}
			
			printf("cas plus petit\n");
		}else if(pres.values[j] == pres.values[j+1]){
			selec+=0.5;
			printf("cas mid\n");
		}else if(pres.values[j] > pres.values[j+1]){
			selec+= pres.values[j] - pres.values[j+1];
			printf("Cas supérieur\n");
		}
	}
	printf("selec : %f\n", selec/count);
	return selec/count;
